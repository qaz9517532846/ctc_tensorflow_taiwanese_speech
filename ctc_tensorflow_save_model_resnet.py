...
modelpath = '/content/drive/My Drive/model'
modelname = 'network_cnn'
...
def run_ctc():
    ...
    with graph.as_default():
        ...
        conv1 = tf.contrib.layers.convolution1d(inputs, num_outputs = num_hidden, kernel_size=3, stride=1, activation_fn=tf.nn.relu)
   
        conv2 = tf.contrib.layers.convolution1d(conv1, num_outputs = num_hidden, kernel_size=3, stride=1, activation_fn=tf.nn.relu)
        
        conv3 = tf.contrib.layers.convolution1d(conv2, num_outputs = num_hidden, kernel_size=3, stride=1, activation_fn=tf.nn.relu)
        
        resnet1 = tf.add(conv1, conv3)
   
        conv4 = tf.contrib.layers.convolution1d(resnet1, num_outputs = num_hidden, kernel_size=3, stride=1, activation_fn=tf.nn.relu)

        conv5 = tf.contrib.layers.convolution1d(conv4, num_outputs = num_hidden, kernel_size=3, stride=1, activation_fn=None)
        
        resnet2 = tf.add(resnet1, conv5)

        logits = tf.transpose(resnet2, (1, 0, 2), name = 'logits')
    ...
    with tf.Session(graph=graph) as session:
        ...
        for curr_epoch in range(num_epochs):
            ...
            if ((curr_epoch+1)%10==0 and (curr_epoch+1)/num_epochs > 0.8):
